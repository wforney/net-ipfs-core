using Google.Protobuf;

namespace Ipfs;

/// <summary>
/// Tests for <see cref="NetworkProtocol"/>.
/// </summary>
[TestClass]
public class NetworkProtocolTest
{
    /// <summary>
    /// Tests that registering a protocol with an existing code throws an exception.
    /// </summary>
    [TestMethod]
    public void Register_Code_Already_Exists() => ExceptionAssert.Throws<ArgumentException>(NetworkProtocol.Register<CodeExists>);

    /// <summary>
    /// Tests that registering a protocol with an existing name throws an exception.
    /// </summary>
    [TestMethod]
    public void Register_Name_Already_Exists() => ExceptionAssert.Throws<ArgumentException>(NetworkProtocol.Register<NameExists>);

    /// <summary>
    /// Tests that the protocol can be converted to a string.
    /// </summary>
    [TestMethod]
    public void Stringing() => Assert.AreEqual("/tcp/8080", new MultiAddress("/tcp/8080").Protocols[0].ToString());

    private class CodeExists : NetworkProtocol
    {
        public override uint Code => 6;
        public override string Name => "x-tcp";

        public override void ReadValue(CodedInputStream stream)
        { }

        public override void ReadValue(TextReader stream)
        { }

        public override void WriteValue(CodedOutputStream stream)
        { }
    }

    private class NameExists : NetworkProtocol
    {
        public override uint Code => 0x7FFF;
        public override string Name => "tcp";

        public override void ReadValue(CodedInputStream stream)
        { }

        public override void ReadValue(TextReader stream)
        { }

        public override void WriteValue(CodedOutputStream stream)
        { }
    }
}

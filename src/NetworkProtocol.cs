...
public abstract class NetworkProtocol
{
    ...
    internal static Dictionary<uint, Type> Codes = [];
...
internal class TcpNetworkProtocol : NetworkProtocol
{
    ...
    {
        byte[] bytes = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)Port));
...
internal class Ipv4NetworkProtocol : IpNetworkProtocol
{
    ...
    public override void ReadValue(CodedInputStream stream)
    {
        byte[] a = stream.ReadSomeBytes(AddressSize)!;
...
internal class OnionNetworkProtocol : NetworkProtocol
{
    ...
    public override void ReadValue(TextReader stream)
    {
        ...
        try
        {
            Port = ushort.Parse(parts[1], CultureInfo.InvariantCulture);
        }
        catch (Exception e)
        {
...
internal class CertHashNetworkProtocol : NetworkProtocol
{
    ...
    public override void ReadValue(CodedInputStream stream) => Value = stream.ReadString();
...
internal class IpcidrNetworkProtocol : NetworkProtocol
{
    ...
    public override void ReadValue(TextReader stream)
    {
        ...
        try
        {
            RoutingPrefix = ushort.Parse(Value!, CultureInfo.InvariantCulture);
        }
        catch (Exception e)
        {
    ...
    public override void WriteValue(CodedOutputStream stream)
    {
        byte[] bytes = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)RoutingPrefix));
        stream.WriteSomeBytes(bytes);
    }
}

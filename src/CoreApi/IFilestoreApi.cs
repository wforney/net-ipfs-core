...
public interface IFilestoreApi
{
    ...
    /// </summary>
    IAsyncEnumerable<FilestoreDuplicate> DupsAsync(CancellationToken token = default);
    ...
    IAsyncEnumerable<FilestoreItem> ListAsync(string? cid = null, bool? fileOrder = null, CancellationToken token = default);
    ...
    /// <param name="fileOrder">Sort the results based on the path of the backing file. Required: no.</param>
    /// <param name="token">A token that can be used to cancel the ongoing operation.</param>
    IAsyncEnumerable<FilestoreItem> VerifyObjectsAsync(string? cid = null, bool? fileOrder = null, CancellationToken token = default);
}

...
/// </summary>
/// <remarks>
/// <b>MultiBase</b> is a protocol for distinguishing base encodings and other simple string
/// encodings. See the <see cref="MultiBaseAlgorithm">registry</see> for supported algorithms.
/// </remarks>
...
public static class MultiBase
{
    ...
    /// <summary>
    /// Converts the specified <see cref="string"/>, which encodes binary data, to an equivalent
    /// 8-bit unsigned integer array.
    /// </summary>
    /// <param name="s">The multi-base string to convert.</param>
    /// <returns>An array of 8-bit unsigned integers that is equivalent to <paramref name="s"/>.</returns>
    /// <exception cref="FormatException">When the <paramref name="s"/> can not be decoded.</exception>
    public static byte[] Decode(string s)
    {
        ...
        _ = MultiBaseAlgorithm.Codes.TryGetValue(s[0], out MultiBaseAlgorithm? alg);
        if (alg is null)
        {
    ...
    /// <param name="algorithmName">The name of the multi-base algorithm to use. See <see href="https://github.com/multiformats/multibase/blob/master/multibase.csv"/>.</param>
    /// <returns>
    /// A <see cref="string"/> starting with the algorithm's <see
    /// cref="MultiBaseAlgorithm.Code"/> and followed by the encoded string representation of
    /// the <paramref name="bytes"/>.
    /// </returns>
    ...
    {
        MultiBaseAlgorithm alg = GetAlgorithm(algorithmName);
    ...
    /// </summary>
    /// <param name="name">
    /// The name of an algorithm, see <see
    /// href="https://github.com/multiformats/multibase/blob/master/multibase.csv"/> for for
    /// IPFS defined names.
    /// </param>
    ...
    {
        return MultiBaseAlgorithm.Names.TryGetValue(name, out MultiBaseAlgorithm? algorithm)
            ? algorithm
